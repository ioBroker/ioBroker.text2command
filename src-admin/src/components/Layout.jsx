...
class Layout extends PureComponent {
    ...
    componentDidMount() {
        this.getDataFromConfig().then(instanceConfig => {
            // convert from rules without ID to rules with ID
            const rulesWithId = instanceConfig.rules
                .map(rule => {
                    ...
                    rule.name =
                        ...
                        window.commands[rule.template]?.name.en;
            ...
            if (JSON.stringify(instanceConfig.rules) !== JSON.stringify(rulesWithId)) {
                ...
                this.props.saveConfig(newConfig).then(isChanged => isChanged && this.getDataFromConfig());
            }
        });
    ...
    handleEdit = () => {
        this.setState({ isEdit: true, isCopy: false }, () => this.handleOpen());
    };

    handleCopy = () => {
        this.setState({ isCopy: true, isEdit: true }, () => this.handleOpen());
    };
    ...
    revertChangesFromConfig = async selectedRule => {
        ...
        const isRuleWasUpdatedGlobally = this.state.unsavedRules[selectedRule.id]?.wasChangedGlobally;
    ...
    saveSettings = async (localeSettings, closeModal) => {
        ...
        await this.props.saveConfig(newConfig);
        this.setState({ settings: localeSettings }, () => closeModal());
    };
    ...
    static getRuleShortData({ _break, template, words, ack, args, name, id }) {
        return {
            words: words || '',
            ack: ack?.default || '',
    ...
    renderModalDialog() {
        ...
        // Use React.createElement to avoid potential JSX parsing issues during build
        return (
            <CreateRuleDialog
                key="modal"
    ...
    render() {
            ...
            return (
                <>
                    ...
                    {this.state.settings && selectedRule ? (
                        ...
                        <Box sx={styles.noRulesText}>{I18n.t('Create a new rule with a "+" on the left')}</Box>
                    )}
        ...
        return (
            <>
                <SplitterLayout
                    key="splitterLayout"
                    style={Utils.getStyle(isLeftBarOpen ? styles.hidden : styles.opened, styles.layout)}
                    primaryMinSize={350}
                    primaryIndex={1}
