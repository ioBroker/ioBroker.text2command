...
export default class Drawer extends Component {
    ...
    onResponse = (id, state) => {
        if (this.state.toast === null) {
            ...
            if (state.val && state.val.match(/^Error\.|^Fehler\.|^Ошибка\./)) {
                this.props.socket.getState(`text2command.${this.props.instance}.error`).then(_state =>
                    this.setState({
                        toast: _state && _state.val ? _state.val : _state.val || '',
                        toastError: true,
                    }),
                );
            } else {
    ...
    handleSubmit = (event, iconPlay) => {
        if ((event && event.key === 'Enter') || iconPlay) {
            this.props.socket
                .setState(`text2command.${this.props.instance}.text`, this.state.textCommand)
    ...
    renderSettingsDialog = () => {
        ...
        return (
            <SettingsDialog
                ...
                onClose={newSettings => {
                    if (newSettings) {
                        this.props.saveSettings(newSettings, () => this.setState({ isSettingsDialogOpen: false }));
                    } else {
    ...
    static createIcons(iconsData) {
        return iconsData.map(({ icon, handler, tooltip }, index) => (
            <Tooltip
                style={{
                    fontSize: 14,
                }}
        ...
        return (
            <Box style={styles.main}>
                ...
                >
                    ...
                    {!isSearchActive && !this.state.alive ? (
                        <Tooltip title={I18n.t('Instance is not running')}>
                            <WarningIcon style={styles.iconNotAlive} />
                        </Tooltip>
                    ) : null}
                </Toolbar>
