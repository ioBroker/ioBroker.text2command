...
class Connection {
	constructor(props) {
		...
		this.props.host =
			...
			(window.location.host &&
				window.location.host.substr(0, window.location.host.indexOf(":")));
	...
	startSocket() {
		...
		this._socket.on("connect", () => {
			this.getAdminVersion().then(version => {
	...
	subscribeState(id, binary, cb) {
		...
		if (!this.statesSubscribes[id]) {
			...
			this.statesSubscribes[id].cbs.indexOf(cb) === -1 &&
				this.statesSubscribes[id].cbs.push(cb);
		}
		if (typeof cb === "function" && this.connected) {
			if (binary) {
				this.getBinaryState(id).then(base64 => cb(id, base64));
			} else {
				this._socket.emit(
					"getForeignStates",
					id,
