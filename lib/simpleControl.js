        ...
        adapter.getForeignState(args[0], (err, state) => {
            ...
            parseTemplates(ack, (err, ack) => {
                if (lang === 'ru') {
                    ...
                    if (tr === 1) {
                        //noinspection JSUnresolvedVariable
                        cb(ack.replace('%s', t).replace('%u', 'градус ' + u).replace('%n', obj.common ? obj.common.name : ''));
                    }
                    else
                    if (tr >= 2 && tr <= 4) {
                        //noinspection JSUnresolvedVariable
...
function extractBinding(format) {
    ...
    if (oid) {
        ...
        for (let p = 0; p < oid.length && p < 50; p++) {
            ...
            let operations = null;
            let isEval = visOid.match(/[\d\w_.]+:[-\d\w_.]+/) || (!visOid.length && parts.length > 0);//(visOid.indexOf(':') !== -1) && (visOid.indexOf('::') === -1);
                ...
                switch (oids[t].operations[k].op) {
                    case 'eval':
                        ...
                        for (let a = 0; a < oids[t].operations[k].arg.length; a++) {
                            ...
                            string += 'let ' + oids[t].operations[k].arg[a].name + ' = "' + value + '";';
                        }
...
function parseTemplatesOld(text, cb) {
    const m = text.match(/{([^{}]+)}/);
    if (m) {
        const id = m[1];
        adapter.getForeignState(id.trim(), (err, state) => {
            text = text.replace('{' + id + '}', state ? state.val : '');
            setImmediate(parseTemplatesOld, text, cb);
        });
    } else {
        cb(null, text);
    }
