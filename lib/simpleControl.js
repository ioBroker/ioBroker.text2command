...
const yes = ['true', 'active', 'ja', 'aktiv', 'да', 'активно', 'активна', 'активный'];
const no = ['false', 'inactive', 'nein', 'inaktiv', 'нет', 'неактивно', 'неактивна', 'неактивный'];
    ...
    adapter.getForeignObject(args[0], (err, obj) => {
        ...
        if (obj) {
            ...
            if (obj.common) {
                ...
                } else if (obj.common.type === 'boolean' || (obj.common.role && obj.common.role.includes('switch'))) {
                    //noinspection JSUnresolvedVariable
                    args[1] = !!args[1];
                }
            }
...
function userQuery(lang, text, args, ack, cb) {
    ...
    getStateAndUnit(id0, lang)
        .then(async data0 => {
            ...
            } else if (!id0 || !data0.obj) {
                adapter.log.warn(`Object "${id0}" does not exist!`);
                return cb('');
            } else if (!data0.state || data0.state.val === null || data0.state.val === undefined) {
                //noinspection JSUnresolvedFunction
                return simpleAnswers.sayIDontKnow(lang, text, args, ack, cb);
            }
            ...
            for (let u = 1; u < parts.length; u++) {
                ...
                if (isEval) {
                    ...
                    if (parse && parse[1]) {
                        ...
                        } else if (parse[1] === 'date' || parse[1] === 'dateinterval') {
                            // date formatting
                            operations = operations || [];
    ...
    for (let t = 0; t < oids.length; t++) {
        if (oids[t].visOid && _values[oids[t].visOid] === undefined) {
            adapter.getForeignState(oids[t].systemOid, (err, state) => {
                ...
                // read settings from systemConfig
                if (
                    systemConfig.isFloatComma &&
                    _values[oids[t].visOid].toString() === parseFloat(_values[oids[t].visOid]).toString()
                ) {
