...
function findFunction(text, lang) {
    ...
    for (let _f in functions.functions) {
        //noinspection JSUnresolvedVariable
        if (!functions.functions.hasOwnProperty(_f)) {
            continue;
        }
...
function getEnum(enumType, text, enums) {
    ...
    for (let t = 0; t < text.length; t++) {
        ...
        for (let rId in enums[enumType]) {
            if (!enums[enumType].hasOwnProperty(rId)) {
                continue;
            }
...
function controlState(lang, obj, value, use01, cb) {
    ...
    // control level
    //noinspection JSUnresolvedVariable
    if (obj.common.type === 'number' || obj.common.role.includes('level')) {
        if (value === true) {
            //noinspection JSUnresolvedVariable
...
function controlDevice(lang, id, value, use01, cb) {
    ...
    adapter.getForeignObject(id, (err, obj) => {
        ...
        if (obj && obj.common && obj.common.role) {
            //noinspection JSUnresolvedVariable
            if (!obj.common.role.includes('switch') &&
                !obj.common.role.includes('state')  &&
                !obj.common.role.includes('level')  &&
                !obj.common.role.includes('blind')) {
                ...
                adapter.getForeignObjects(id + '.*', (err, list) => {
                    ...
                    if (list) {
                        ...
                        for (id in list) {
                            ...
                            //noinspection JSUnresolvedVariable
                            if (list[id].type === 'state' && list[id].common.write !== false &&
                                (list[id].common.role.includes('switch') ||
                                 list[id].common.role.includes('state') ||
                                 list[id].common.role.includes('level'))
                            ) {
                                //noinspection JSUnresolvedVariable
                                if (value === true  && list[id].common.max !== undefined) {
                                    //noinspection JSUnresolvedVariable
...
function extractOnOff(lang, text) {
    ...
    if (lang === 'ru') {
        ...
        if (cmdWords.includes('выключи') || cmdWords.includes('выключить')) {
            valPercent = false;
        }
    }
    else if (lang === 'de') {
        ...
        if (cmdWords.includes('an') || cmdWords.includes('ein') || cmdWords.includes('einmachen') || cmdWords.includes('einschalten')) {
            valPercent = true;
        }
    }
    else if (lang === 'en') {
...
function extractBlindCmd(lang, text) {
    ...
    if (lang === 'ru') {
        ...
        // test operation
        if (text.includes(' auf') ||
            text.includes('hoch') ||
            text.includes('öffnen') ||
            text.includes('aufmachen')) {
            valPercent = 100;
        } else
        if (text.includes('zumachen') ||
            text.includes(' zu') ||
