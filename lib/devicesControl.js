...
const rooms         = require(__dirname + '/rooms');
const functions     = require(__dirname + '/functions');
...
function controlState(obj, value, use01, cb) {
    //noinspection JSUnresolvedVariable
...
function controlDevice(id, value, use01, cb) {
    ...
    adapter.getForeignObject(id, function (err, obj) {
        ...
        if (obj && obj.common && obj.common.role) {
            ...
            if (obj.type !== 'state') {
                    ...
                    if (list) {
                        ...
                        for (id in list) {
                            ...
                            if (list[id].type === 'state' && list[id].common.write !== false &&
                                ...
                                controlState(list[id], value, use01, cb);
                                return;
                            }
                        }
                ...
                controlState(obj, value, use01, cb);
            }
...
function controlByFunctionHelper(lang, text, args, ack, cb, sFunction, extractValue, generateAnswer) {
    ...
    if (sRoom === 'everywhere') {
        for (let f = 0; f < devicesInFunction.length; f++) {
            count++;
            controlDevice(devicesInFunction[f], valPercent, (args && args[0]), function (err) {
        ...
        for (let r = 0; r < devicesInRoom.length; r++) {
            if (devicesInFunction.indexOf(devicesInRoom[r]) !== -1) {
                count++;
                controlDevice(devicesInRoom[r], valPercent, (args && args[0]), function (err) {
                    if (!err) anyControlled = true;
                    setTimeout(function () {
                        if (!--count) {
                            if (!anyControlled) {
